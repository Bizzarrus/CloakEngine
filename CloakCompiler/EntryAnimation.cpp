...
namespace CloakCompiler {
	namespace Engine {
		namespace Image {
			namespace v1008 {
				namespace EntryAnimation_v1 {
					constexpr CloakEngine::Files::FileType g_TempFileType{ "ImageTemp","CEIT",1008 };

					CLOAK_CALL EntryAnimation::EntryAnimation(In const API::Image::v1008::IMAGE_INFO_ANIMATION& img, In const ResponseCaller& response, In size_t imgNum, In uint8_t qualityOffset, In API::Image::v1008::Heap heap)
					{
						...
						for (size_t a = 0; a < imgCount && m_err == false; a++)
						{
							if (img.Images[a].ShowTime > 0)
							{
								...
								{
									CalculateMipMaps(img.MaxMipMapCount, qualityOffset, i, img.Images[a].Meta, &m_textures[m_texCount].Texture);
									if (m_textures[m_texCount].Texture.levelCount <= qualityOffset) { SendResponse(response, API::Image::v1008::RespondeState::ERR_QUALITY, imgNum, "Image is not large enought to support this quality setting", a + 1); m_err = true; }
									m_textures[m_texCount].ShowTime = img.Images[a].ShowTime;
									m_layerCount = max(m_layerCount, m_textures[m_texCount].Texture.levelCount);
									m_texCount++;
								}
							}
							else
							{
								SendResponse(response, API::Image::v1008::RespondeState::WARN_SHOWTIME, imgNum, "Image will not be shown in animation", a + 1);
							}
						}
						if (m_texCount == 0) { SendResponse(response, API::Image::v1008::RespondeState::ERR_NO_IMGS, imgNum, "No textures have a show time greater than zero!"); m_err = true; }
						if (m_err == false)
						{
							m_layerCount++;
					...
					CLOAK_CALL EntryAnimation::~EntryAnimation()
					{
						DeleteArray(m_textures);
						DeleteArray(m_metas);
						DeleteArray(m_layers);
					}
					bool CLOAK_CALL_THIS EntryAnimation::GotError() const { return m_err; }
					void CLOAK_CALL_THIS EntryAnimation::CheckTemp(In const API::EncodeDesc& encode, In const BitInfoList& bitInfos, In const ResponseCaller& response, In size_t imgNum)
					{
						if (m_err) { return; }
						...
						if (read != nullptr)
						{
							...
							if (suc) { suc = (static_cast<API::Image::ImageInfoType>(read->ReadBits(3)) == API::Image::ImageInfoType::ANIMATION); }
							...
							for (size_t a = 0; a < m_texCount && suc; a++)
							{
								...
								if (suc) { suc = (static_cast<uint32_t>(read->ReadBits(10)) == m_textures[a].Texture.levelCount); }
								if (suc) { suc = (read->ReadDynamic() == m_textures[a].ShowTime); }
							}
					...
					void CLOAK_CALL_THIS EntryAnimation::WriteTemp(In const API::EncodeDesc& encode, In const BitInfoList& bitInfos, In const ResponseCaller& response, In size_t imgNum)
					{
						...
						if (write != nullptr)
						{
							...
							write->WriteBits(3, static_cast<uint8_t>(API::Image::ImageInfoType::ANIMATION));
							...
							for (size_t a = 0; a < m_texCount; a++)
							{
								...
								write->WriteBits(10, m_textures[a].Texture.levelCount);
								write->WriteDynamic(m_textures[a].ShowTime);
							}
					...
					void CLOAK_CALL_THIS EntryAnimation::Encode(In const BitInfoList& bitInfos, In const ResponseCaller& response, In size_t imgNum)
					{
						if (m_useTemp == false && m_err == false)
						{
							...
							write->WriteBits(3, 3); //Animation type
							write->WriteDynamic(m_texCount);
							write->WriteBits(2, m_qualityOffset);
							write->WriteBits(1, m_alphaMap ? 1 : 0);
							write->WriteBits(4, static_cast<uint8_t>(m_channel));
							...
							const UINT lW = m_textures[0].Texture.levels[0].Size.W;
							const UINT lH = m_textures[0].Texture.levels[0].Size.H;
							write->WriteBits(10, m_textures[0].Texture.levelCount);
							write->WriteDynamic(lW);
							write->WriteDynamic(lH);
							bool* hr = NewArray(bool, m_texCount);
							for (size_t a = 0; a < m_texCount; a++)
							{
								hr[a] = CheckHR(m_textures[a].Texture, m_channel);
								...
								if (m_alphaMap == true) { EncodeAlphaMap(write, m_textures[a].Texture.levels[0]); }
								write->WriteDynamic(m_textures[a].ShowTime);
								if (m_textures[a].Texture.levels[0].Size.W != lW || m_textures[a].Texture.levels[0].Size.H != lH)
								{
							...
							for (size_t a = 1; a < m_layerCount && m_err == false; a++)
							{
								...
								{
									if (m_textures[b].Texture.levelCount >= a)
									{
										MipMapLevel* prevTex = nullptr;
										MipMapLevel* prevMip = nullptr;
										if (prev < m_texCount) { prevTex = &m_textures[prev].Texture.levels[a - 1]; }
										if (m_textures[b].Texture.levelCount > a) { prevMip = &m_textures[b].Texture.levels[a]; }
										m_err = m_err || EncodeTexture(write, bitInfos, m_textures[b].Texture.levels[a - 1], m_metas[b], response, imgNum, b, m_channel, hr[b], prevTex, prevMip);
										prev = b;
									}
								}
					...
					void CLOAK_CALL_THIS EntryAnimation::WriteHeader(In const CE::RefPointer<CloakEngine::Files::IWriter>& write)
					{
						if (m_err == false)
						{
					...
					size_t CLOAK_CALL_THIS EntryAnimation::GetMaxLayerLength()
					{
						if (m_err == false) { return m_layerCount - (1 + m_qualityOffset); }
						return 0;
					}
					void CLOAK_CALL_THIS EntryAnimation::WriteLayer(In const CE::RefPointer<CloakEngine::Files::IWriter>& write, In size_t layer)
					{
						if (m_err == false)
						{
					...
					void CLOAK_CALL_THIS EntryAnimation::WriteLibFiles(In const CE::RefPointer<CloakEngine::Files::IWriter>& file, In const API::Image::LibInfo& lib, In const BitInfo& bits, In size_t entryID, In const Engine::Lib::CMakeFile& cmake, In const CE::Global::Task& finishTask)
					{
